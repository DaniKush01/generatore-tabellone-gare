...
require_once __DIR__ . '/config/db_connect.php';
  ...
  if ($method === 'GET') {
    ...
    // non-admin: forziamo la propria palestra
    if (!is_admin()) {
      $_GET['gym'] = user_gym();
    }
    ...
    $map = [
      ...
      // filtra per pagato solo se la colonna esiste
  ...
  /* ── helper: bind con tipo corretto ── */
  function bind_val(PDOStatement $st, string $name, $val): void {
    if ($val === null) { $st->bindValue($name, null, PDO::PARAM_NULL); return; }
    if (is_int($val))  { $st->bindValue($name, $val, PDO::PARAM_INT);  return; }
    // per DECIMAL/float usiamo string, va benissimo per MySQL
  ...
  if ($method === 'PATCH') {
    ...
    // Mappa trasformazioni / sanificazioni
    $NON_PESATE = ['Dimostrazione'=>1, 'Kata'=>1, 'Kata Kobudo'=>1];

    $allowed = [
      'nome'         => function($v){ return trim((string)$v); },
      'cognome'      => function($v){ return trim((string)$v); },
      'sesso'        => function($v){ $v = strtoupper(trim((string)$v)); return ($v==='M'||$v==='F')?$v:''; },
      'eta'          => function($v){ return ($v === '' || $v === null) ? null : max(0, (int)$v); },
      'peso'         => function($v) use ($in, $NON_PESATE){
                          $comp = isset($in['competizione']) ? (string)$in['competizione'] : null;
                          // se non viene passato, prendo competizione attuale dal DB? (per semplicità, accettiamo null come "mantieni" o imposta)
                          if ($v === '' || $v === null) return null; // permetti NULL
                          $f = (float)str_replace(',', '.', (string)$v);
                          return $f < 0 ? 0 : $f;
                        },
      'cintura'      => function($v){ return trim((string)$v); },
      'competizione' => function($v){ return trim((string)$v); },
    ];
    if ($hasPrezzo) {
    ...
    // Solo admin può cambiare palestra
    if (is_admin()) {
      $allowed['palestra'] = function($v){ return trim((string)$v); };
    }
    ...
    if (!$set) respond(['ok'=>true]); // niente da aggiornare

    $sql = "UPDATE atleti SET ".implode(', ', $set)." WHERE id = :id";
    $stmt = $pdo->prepare($sql);
    foreach ($params as $name => $val) bind_val($stmt, $name, $val);
    $stmt->execute();
  ...
  respond(['error' => $DEBUG ? $e->getMessage() : 'Errore server'], 500);
}
