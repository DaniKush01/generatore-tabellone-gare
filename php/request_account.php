  ...
  if ($hasPHPMailer) {
    ...
    try {
      if (!empty($SMTP['enabled'])) {
        $mail->isSMTP();
        $mail->Host = $SMTP['host']; $mail->SMTPAuth = true;
        $mail->Username=$SMTP['user']; $mail->Password=$SMTP['pass'];
        $mail->SMTPSecure=$SMTP['secure']; $mail->Port=(int)$SMTP['port'];
      } else $mail->isMail();
      $mail->CharSet='UTF-8';
...
if (!$palestra || !$referente || !$email) respond(['error'=>'Campi obbligatori mancanti'],400);
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) respond(['error'=>'Email non valida'],400);

$html = '
  <div style="font-family:Arial,Helvetica,sans-serif;line-height:1.5;color:#111">
    <h2 style="margin:0 0 8px">Richiesta credenziali</h2>
...
$ok = sendMailGeneric($ADMIN_EMAIL, 'Richiesta credenziali - '.$palestra, $html);
